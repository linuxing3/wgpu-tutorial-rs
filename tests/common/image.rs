...
pub fn calc_difference(lhs : u8, rhs : u8) -> u8 { (lhs as i16 - rhs as i16).unsigned_abs() as u8 }
...
fn copy_texture_to_buffer_with_aspect(
    ...
    let (block_width, block_height) = texture.format().describe().block_dimensions;
        ...
        ImageCopyBuffer {
            ...
            layout : ImageDataLayout {
                ...
                bytes_per_row : std::num::NonZeroU32::new(
                    (texture.width() / block_width as u32) * block_size,
                ),
                rows_per_image : std::num::NonZeroU32::new(texture.height() / block_height as u32),
            },
        },
        texture
            .size()
...
impl ReadbackBuffers {
    pub fn new(device : &Device, texture : &Texture) -> Self {
        ...
        let base_size = (texture.width() / block_width as u32)
            * (texture.height() / block_height as u32)
            * texture.depth_or_array_layers();

        let buffer_size = base_size * (texture.format().describe().block_size as u32);
